...
// FIND THE SUM IN THE LIST USING RECURSION
function sum(list) {
...
function sumHelper(list, p) {
    ...
    return p.element() + sumHelper(list, list.after(p));

}

// FIND THE MAXIMUM ELEMENT IN THE LIST USING RECURSION
function findMax(list) {
    return maxHelper(list, list.first())
}
function maxHelper(list, p) {
    if (list.size() == 1) {
        return p.element();
    }
    if (p.element() < list.after(p).element()) {
        list.remove(p);
    }
    else {
...
function sum(List) {
    ...
    while(!List.isLast(first)){
       sum += first.element();
       first = List.after(first);
    }
     sum += List.last().element();
     return sum;
}
...
function  findMax(List) {
    let first = List.first();
    let max = List.first().element();
    while(!List.isLast(first)){
        let second = List.after(first).element();
        if(max < second){
            max = second;
        }
        first = List.after(first);
    }
    if( max<List.last().element()){
        max = List.last().element();
    }
    return max;
}
var tst0 = new List();
